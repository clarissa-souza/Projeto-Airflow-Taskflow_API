...
def tratamento():
  ...
  df = df.groupby(['ano','regiao','UF','grau_academico']).sum()
  return df
...
with DAG('extensao_airflow', start_date = datetime(2022,11,8),schedule_interval='30 * * * *', catchup=False, default_args=default_args) as dag:
